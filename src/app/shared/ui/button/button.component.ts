...
export class ButtonComponent {
  ...
  readonly classes = computed(() => {
    const base =
      'inline-flex cursor-pointer items-center justify-center gap-1.5 rounded-xl font-semibold transition focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60';
    const size = this.size() === 'sm' ? 'px-2.5 py-1.5 text-xs' : 'px-4 py-2.5 text-xs';
