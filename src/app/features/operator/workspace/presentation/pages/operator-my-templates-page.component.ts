...
interface OperatorTemplateView {
  ...
  customInputs: readonly OperatorCustomInputView[];
...
interface OperatorLatestResponseView {
  ...
  customInputs: readonly OperatorLatestCustomInputValueView[];
...
export class OperatorMyTemplatesPageComponent implements OnInit, OnDestroy {
  ...
  readonly customInputValues = signal<Record<string, string>>({});
  readonly touchedCustomInputIds = signal<ReadonlySet<string>>(new Set());
  ...
  readonly allCustomInputsValid = computed(() => {
    const template = this.activeTemplate();
    return (
      template === null ||
      template.customInputs.every((customInput) => this.customInputError(customInput).length === 0)
    );
  });
  readonly allQuestionsValid = computed(() => {
    return (
      ...
      this.allCustomInputsValid()
    );
  });
  readonly currentQuestion = computed(() => {
    if (!this.activeTemplate() || this.summaryOpen() || this.customInputsOpen()) {
      return null;
    }
  ...
  closeResponseWizard(): void {
    ...
    this.touchedCustomInputIds.set(new Set());
    this.answers.set({});
  ...
  goToPreviousStep(): void {
    ...
    const template = this.activeTemplate();
    if (this.currentQuestionIndex() === 0 && template && template.customInputs.length > 0) {
      this.customInputsOpen.set(true);
      return;
    }
  ...
  goToNextStep(): void {
    if (this.customInputsOpen()) {
      this.markAllCustomInputsTouched();
      if (!this.allCustomInputsValid()) {
        return;
      }
      this.customInputsOpen.set(false);
      this.currentQuestionIndex.set(0);
  ...
  goToQuestion(index: number): void {
    ...
    if (template.customInputs.length > 0 && !this.allCustomInputsValid()) {
      this.markAllCustomInputsTouched();
      this.customInputsOpen.set(true);
      return;
    }
  ...
  submitResponse(): void {
    ...
    this.operatorTemplatesStore.submitTemplateResponse(
      ...
      this.toCustomInputSubmissions(template),
      (response) => {
        this.submittedResponse.set(response);
        this.responseSubmitted.set(true);
        this.customInputsOpen.set(false);
        this.summaryOpen.set(true);
        this.expandedLatestResponseTemplateIds.update(
          (templateIds) => new Set(templateIds).add(template.templateId),
        );
      },
      (submitError) => {
  ...
  openCustomInputsStep(): void {
    const template = this.activeTemplate();
    if (!template || template.customInputs.length === 0 || this.responseSubmitted()) {
      return;
    }
  ...
  updateCustomInputValue(customInput: OperatorCustomInputView, event: Event): void {
    const target = event.target;
    const value = target instanceof HTMLInputElement ? target.value : '';
    this.customInputValues.update((values) => ({
      ...values,
      [customInput.customInputId]: value,
    }));
    this.markCustomInputTouched(customInput.customInputId);
  }

  customInputValue(customInput: OperatorCustomInputView): string {
    return this.customInputValues()[customInput.customInputId] ?? '';
  }

  visibleCustomInputError(customInput: OperatorCustomInputView): string {
    return this.touchedCustomInputIds().has(customInput.customInputId)
      ? this.customInputError(customInput)
      : '';
  }
  ...
  private toTemplateView(template: OperatorAssignedTemplate): OperatorTemplateView {
    ...
    return {
      ...
      customInputs: template.customInputs.map((customInput) =>
        this.toCustomInputView(customInput, isArabic),
      ),
      questions,
      questionConditions,
    };
  ...
  private toCustomInputView(
    customInput: OperatorAssignedTemplateCustomInput,
    isArabic: boolean,
  ...
  private toLatestCustomInputValueView(
    ...
    return {
      ...
      value:
        customInput.type === 2
          ? customInput.integerValue?.toString() ?? ''
  ...
  private resetResponseState(
    ...
    this.customInputValues.set(this.createInitialCustomInputValues(template, hydrateLatestResponse));
    this.touchedCustomInputIds.set(new Set());
    this.touchedQuestionIds.set(new Set());
    this.currentQuestionIndex.set(0);
    this.customInputsOpen.set(template.customInputs.length > 0);
    this.summaryOpen.set(false);
    this.responseSubmitted.set(false);
    this.submittedResponse.set(null);
    this.clearHiddenQuestionState(template);
  ...
  private createInitialCustomInputValues(
    template: OperatorTemplateView,
    hydrateLatestResponse: boolean,
  ): Record<string, string> {
    const latestValues = new Map(
      (hydrateLatestResponse ? template.latestResponseSource?.customInputs ?? [] : []).map(
        (customInput) => [
    ...
    return Object.fromEntries(
      template.customInputs.map((customInput) => [
        customInput.customInputId,
        latestValues.get(customInput.customInputId) ?? '',
      ]),
    );
  }
  ...
  private markCustomInputTouched(customInputId: string): void {
    this.touchedCustomInputIds.update(
      (customInputIds) => new Set([...customInputIds, customInputId]),
    );
  }

  private markAllCustomInputsTouched(): void {
    const template = this.activeTemplate();
    this.touchedCustomInputIds.set(
      new Set(template?.customInputs.map((customInput) => customInput.customInputId) ?? []),
    );
  }
  ...
  private customInputError(customInput: OperatorCustomInputView): string {
    const value = this.customInputValue(customInput).trim();

    if (customInput.isRequired && value.length === 0) {
    ...
    if (customInput.type === 1) {
      if (customInput.minLength !== null && value.length < customInput.minLength) {
        return 'operatorTemplates.customInputMinLength';
      }

      if (customInput.maxLength !== null && value.length > customInput.maxLength) {
    ...
    if (customInput.minValue !== null && numericValue < customInput.minValue) {
      return 'operatorTemplates.customInputMinValue';
    }

    if (customInput.maxValue !== null && numericValue > customInput.maxValue) {
  ...
  private toCustomInputSubmissions(
    template: OperatorTemplateView,
  ): readonly OperatorTemplateCustomInputSubmission[] {
    const submittedCustomInputIds = new Set<string>();
    const submissions: OperatorTemplateCustomInputSubmission[] = [];

    for (const customInput of template.customInputs) {
      if (submittedCustomInputIds.has(customInput.customInputId)) {
        continue;
      }
      ...
      submittedCustomInputIds.add(customInput.customInputId);
      submissions.push({
        customInputId: customInput.customInputId,
        value,
      });
